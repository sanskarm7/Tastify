 ...
 */
final class SpotifyTest: ObservableObject {
    
    private static let clientId: String = {
        if let clientId = ProcessInfo.processInfo
                .environment["9c0fd040bed64867af0d344e5bed6254"] {
            return clientId
        }
    ...
    private static let clientSecret: String = {
        if let clientSecret = ProcessInfo.processInfo
                .environment["Cc1ea3ade2b1840eea2cc5eb4f33f6b74"] {
            return clientSecret
        }
    ...
    let keychain = Keychain(service: "com.sanskar.Music-App")
    
    /// An instance of `SpotifyAPI` that you use to make requests to the Spotify
    /// web API.
    let api = SpotifyAPI(
        authorizationManager: AuthorizationCodeFlowManager(
            clientId: SpotifyTest.clientId,
            clientSecret: SpotifyTest.clientSecret
        )
    )
